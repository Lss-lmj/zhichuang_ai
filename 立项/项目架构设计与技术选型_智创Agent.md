...
3. 学生端成长闭环、知识库、竞赛推荐、组队推荐如何落到模块和数据表。
4. 如何保证可追溯、可复现、可演示、可扩展。
...
| 目标 | 说明 |
|---|---|
| 主线清晰 | 以学生端成长闭环和教师端作业分析闭环为主线 |
| 可信可追溯 | 事实性回答必须有知识库来源、引用片段和更新时间 |
| 快速可演示 | 采用成熟 Web 技术栈和轻量部署方式，保证短周期内完成 Demo |
| 数据可控 | 系统面向学校真实使用，支持课程、作业、学生提交等真实数据按权限访问 |
| 模型可替换 | 大模型通过 Provider Adapter 接入，可选择通用大模型、开源大模型、OpenAI-compatible 服务或本地模型 |
| 推荐可解释 | 推荐结果必须展示评分依据、证据来源、短板和风险提醒 |
| 比赛可复现 | 代码、样例数据、部署说明、测试用例和演示账号可一起提交 |
...
| 个人数据敏感 | RBAC 权限控制、组队主动授权、教师按课程和班级授权访问 |
| 教师学情诊断是核心教学场景 | 课程作业代码分析和教师看板进入 MVP，竞赛候选筛选等能力后续扩展 |
...
| 模块 | 子模块 | MVP 优先级 | 说明 |
|---|---|---|---|
| 用户与权限 | 登录、角色、RBAC、演示账号 | P0 | 支持学生、教师、管理员 |
...
| 代码与项目分析 | 代码上传、仓库解析、能力证据提取、报告生成 | P0 | 支撑作业分析、画像证据和项目能力判断 |
| 测试评测 | 测试案例、输出记录、人工评分、效果报告 | P0 | 服务技术验证和演示复现 |
| 教师看板 | 作业分析看板、学生报告、班级能力分布、导出 | P0 | 教师直接查看系统分析结果，不做审核流 |
...
| 层级 | 推荐选型 | 选择理由 |
|---|---|---|
| 前端策略 | Web 首版：React + TypeScript + Vite；鸿蒙端：ArkTS + ArkUI | Web 端适合快速完成 Demo，鸿蒙端适合参加鸿蒙类比赛，二者复用同一套后端 API |
...
| 部署 | Docker Compose + Nginx | 一键部署，适合 Demo、答辩和代码复现 |
...
| Elasticsearch / OpenSearch | 检索能力强，但首版运维较重；可作为后续规模化升级 |
...
当前 Web 前端使用 React + Vite，适合快速完成浏览器 Demo、后台管理和教师看板，但它不等于鸿蒙原生应用，不能直接迁移成 ArkTS/ArkUI 页面。
...
/teacher/assignments           课程作业分析看板
/teacher/reports               学生作业分析报告
...
| 演示模式 | 准备学生、教师、管理员三个演示账号，一键加载样例数据 |
...
教师看板和管理端仍保留在 Web 端，保证课程作业分析和班级报告的展示效率。
...
| 作业看板 | `GET /api/teacher/assignments/{id}/dashboard` | 查看一次课程作业的提交情况、分数分布和常见问题 |
| 作业报告 | `GET /api/teacher/submissions/{id}/report` | 查看单个学生作业代码分析报告 |
...
| 任务 | 使用 LangGraph 的原因 |
|---|---|
| 学习路径规划 | 需要目标澄清、资料检索、计划生成、计划评估、用户保存或调整、后续复盘 |
...
| 定期复盘 | 需要读取历史任务、识别延期、重新规划、生成提醒和下一步动作 |
| 作业报告生成 | 需要对一次作业或一个项目执行多 Agent 分析，并沉淀可供教师查看的报告 |
...
| 教师看板 | `TeacherDashboardGraph` | 权限检查、作业报告聚合、班级分布、共性问题摘要 | 班级作业分析看板 |
...
| `assignments` | 课程作业 | teacher_id、course_id、title、deadline、rubric_json |
| `assignment_submissions` | 作业提交 | assignment_id、student_id、project_id、submitted_at、status |
...
| 教师 | 授权班级统计信息、课程作业分析看板、学生作业分析报告、公开知识库 |
...
学生提交课程作业代码或项目压缩包
  ...
  -> 功能完成度 Agent 对照作业要求检查实现情况
  -> 代码质量 Agent 分析可读性、复杂度、异常处理、重复代码
  -> 工程规范 Agent 检查 README、依赖管理、运行方式、测试情况
  -> 能力证据 Agent 提取可支撑画像的技能证据
  -> 报告生成 Agent 汇总多维度评分、问题、亮点和改进建议
  -> 写入 code_analysis_reports
  -> 教师端看板和学生端报告同步可见
```

作业分析报告不是教师审核流，而是系统对提交物的结构化分析结果。教师进入课程作业看板后，直接查看班级整体情况和单个学生报告。

多维度评分建议：

| 维度 | 说明 |
|---|---|
| 功能完成度 | 是否覆盖作业要求、核心功能是否可运行 |
...
教师端定位为分析结果消费端，服务课程作业讲评和学情掌握，不做审核流。
...
4. 通过初始化脚本写入演示账号、样例画像、样例竞赛和样例知识库。
...
| 教师端范围膨胀 | 教师端从作业分析扩散到复杂管理系统 | MVP 只做课程作业分析看板和学生报告，竞赛筛选等后续扩展 |
| 部署复杂 | 现场复现困难 | Docker Compose 一键部署，准备公网 Demo 可用的示例课程数据 |
| 权限配置错误 | 学生、教师、管理员看到超出授权范围的数据 | RBAC、课程班级授权、操作审计和演示账号隔离 |
