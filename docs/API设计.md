...
```http
Authorization: Bearer demo-token-admin_001
```

响应包含账号角色、默认视图、授权课程、授权班级和可访问模块。学生或教师账号访问返回 `403`。
...
```json
{
  ...
  "agent_task_id": "agent_task_assignment_abc123",
...
响应返回入库后的资料卡片、是否可检索和处理结果。新增资料写入 SQLite `knowledge_documents` 表，随后可通过资料清单、版本记录和检索接口验证。
...
接口从 SQLite `evaluation_cases` 和 `evaluation_records` 读取管理员维护的案例和记录；首次启动会写入首批内置评测样例。
...
新增案例写入 SQLite `evaluation_cases`，后续可在评测看板和案例清单中查看。
...
新增记录写入 SQLite `evaluation_records`，用于持续追踪系统输出质量。
